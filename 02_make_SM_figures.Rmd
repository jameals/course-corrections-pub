...
# filter to the min, median, and max per diff sq for Hcm

t_update_df_stoch_wide_Hcm_median <- t_update_df_stoch_wide %>%
  filter(strategy != 'SQ') %>%
  group_by(change_type, change_par, direction, change_par_direction, strategy_label) %>%
  summarise(
    Hcm_per_diff_sq = median(Hcm_per_diff_sq)
    ) %>%
  mutate(metric = 'Hcm', statistic = 'median')

t_update_df_stoch_wide_Hcm_max <- t_update_df_stoch_wide %>%
  filter(strategy != 'SQ') %>%
  group_by(change_type, change_par, direction, change_par_direction, strategy_label) %>%
  slice_max(Hcm_per_diff_sq) %>%
  mutate(metric = 'Hcm', statistic = 'max') %>%
  dplyr::select(change_type, change_par, direction, change_par_direction, strategy_label, Hcm_per_diff_sq, metric, statistic)

t_update_df_stoch_wide_Hcm_min <- t_update_df_stoch_wide %>%
  filter(strategy != 'SQ') %>%
  group_by(change_type, change_par, direction, change_par_direction, strategy_label) %>%
  slice_min(Hcm_per_diff_sq) %>%
  mutate(metric = 'Hcm', statistic = 'min') %>%
  dplyr::select(change_type, change_par, direction, change_par_direction, strategy_label, Hcm_per_diff_sq, metric, statistic)

t_update_df_stoch_wide_Hcm <- t_update_df_stoch_wide_Hcm_median %>% 
  bind_rows(t_update_df_stoch_wide_Hcm_max) %>%
  bind_rows(t_update_df_stoch_wide_Hcm_min) %>%
  pivot_wider(
    names_from = statistic, values_from = Hcm_per_diff_sq
    )

t_update_df_stoch_wide <- t_update_df_stoch_wide_Bmn %>%
  bind_rows(t_update_df_stoch_wide_Hcm)
...
df_ylim_0_stoch <- c(
    ...
    min_Hcm_per_diff_sq = min(t_update_df_stoch_wide_Hcm_min$Hcm_per_diff_sq),
    max_Hcm_per_diff_sq = max(t_update_df_stoch_wide_Hcm_max$Hcm_per_diff_sq)
  )

df_ylim_stoch <- tibble(metric = c('Bmn','Hcm'), 
                    y_ul = c(
                      df_ylim_0_stoch[['max_Bmn_per_diff_sq']],
                      df_ylim_0_stoch[['max_Hcm_per_diff_sq']]
                      ),
                    y_ll = c(
                      df_ylim_0_stoch[['min_Bmn_per_diff_sq']],
                      df_ylim_0_stoch[['min_Hcm_per_diff_sq']]
                      )
                    )
...
plot_Hcm_d <- ggplot(t_update_df_stoch_wide %>% 
         filter(direction == 'decrease') %>%
           filter(metric == 'Hcm'),
            aes(x=strategy_label, y=median, fill=change_par, group=change_par)
       ) +
  ...
  scale_y_continuous(
    limits = c(df_ylim_stoch %>% filter(metric == 'Hcm') %>% pull(y_ll)-10, df_ylim_stoch %>% filter(metric == 'Hcm') %>% pull(y_ul)),
    breaks = scales::breaks_pretty(10)) +
  scale_colour_manual(
    name = str_wrap('Climate-Induced Decline',20),
    values = pal_d,
...
plot_Hcm_i <- ggplot(t_update_df_stoch_wide %>% 
         filter(direction == 'increase') %>%
           filter(metric == 'Hcm'),
            aes(x=strategy_label, y=median, fill=change_par, group=change_par)
       ) +
